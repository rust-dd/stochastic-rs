...
- **85+ stochastic processes** — 31 diffusions (OU, CIR, GBM, CEV, CKLS, Aït-Sahalia, Pearson, Jacobi, regime-switching, …), 15 jump processes (Merton, Kou, CGMY, bilateral gamma, …), 9 stochastic-volatility models (Heston, SABR, Bergomi, rough Bergomi, HKDE, …), 12 short-rate / HJM models (Vasicek, CIR, Hull-White, HJM, Wu-Zhang, Duffie-Kan, Ho-Lee, fractional Vasicek, …), plus base processes (fBM, fGN, Poisson, Hawkes, Lévy, LFSM, …) and parallel uncoupled-rate samplers (`Bgm` — Euler-stepped multiplicative martingales, see its module doc; **not** a coupled BGM/LMM). Each carries a generic-precision `ProcessExt<T>` impl and CUDA / SIMD acceleration where applicable.
