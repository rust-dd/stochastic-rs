...
- [x] **`Vec<Vec<f64>>` → `Array2<T>`** — `portfolio/data.rs` API now returns `Array2<f64>` (`align_return_series`, `correlation_matrix`, `covariance_matrix`, `corr_from_cov`). `portfolio/momentum.rs` uses `Array2` internally with adapter at the optimizer boundary. `calibration/rbergomi.rs` `MsoeEngine.chol_l` is now `Array2<f64>`. `pricing/dupire.rs` and `pricing/breeden_litzenberger.rs` were already `Array2`-based. `portfolio/optimizers.rs` keeps `Vec<Vec<f64>>` internals (separate scope: `argmin` cost-fn structures hold them, not directly user-facing).
...
- [x] **Inline unit tests** — added `#[cfg(test)]` modules to `quant/calendar/{day_count,holiday,business_day,schedule}.rs`, `quant/bonds/{vasicek,cir,hull_white}.rs`, `quant/curves/{discount_curve,nelson_siegel,svensson,types,interpolation,bootstrap,multi_curve}.rs`, `quant/fx/{currency,forward,quoting}.rs`, and `stochastic/interest/{vasicek,fractional_vasicek,bgm,duffie_kan,duffie_kan_jump_exp,hull_white,hull_white_2f,adg}.rs`. Tests cover closed-form sanity checks (e.g. `P(t,t)=1` for ZCBs, CIP for FX forwards) and sample-shape smoke tests for stochastic processes. Integration tests in `tests/` complement.
...
- [x] **Naming conventions** — `RBergomiCalibrationConfig` / `HestonNMLECEKFConfig` confirmed correct (`Config` for algorithm settings + initial Params, separate from model `*Params` types). Renamed `mod_duffie_kan.rs` → `duffie_kan_jump_exp.rs` (matches `DuffieKanJumpExp` struct), `fvasicek.rs` → `fractional_vasicek.rs`. `_2f` suffix kept as standard two-factor marker.
