...
### CUDA oxide experimental (optional)

Experimental FGN/fBM backend using NVlabs `cuda-oxide` Rust kernels for the full
GPU pipeline: Philox/Box-Muller generation, eigenvalue scaling, radix-2 FFT, and
output extraction. This is Linux/CUDA-only and must be built with `cargo oxide`
rather than plain `cargo`.

```bash
cargo install --git https://github.com/NVlabs/cuda-oxide.git cargo-oxide
cargo oxide run --features cuda-oxide-experimental --bin fgn_cuda_oxide_smoke
```

The module name must match the `.ptx` / `.ll` artifact generated by
`cargo oxide`; downstream binaries can pass an explicit module name with
`Fgn::sample_cuda_oxide_with_module`.
