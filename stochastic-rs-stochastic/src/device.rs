...
/// | `Accelerate` (`accelerate` feature) | Same guarantee as `Cpu` (vDSP is a CPU framework), reached by a different internal mechanism — see each backend's own `generate_batch` impl doc. |
...
/// mechanism `Cpu`/`Accelerate` use for the guarantee above; the GPU backends
