...
description: Compile-time GPU / accelerator backend selection for the fractional family via `.on::<B>()` — the `Backend` marker, the `FgnBackend` capability, and zero-cost dispatch.
category: concept
since: 2.4.0
status: stable
---
