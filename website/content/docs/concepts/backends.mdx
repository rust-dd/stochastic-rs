...
`Metal` and `Cubecl` for `f32`, the CPU devices for both. `Fgn<f64>` or
...
| `Cubecl`      | `cubecl-cuda` / `cubecl-wgpu`    | `cubecl` portable kernels, on the runtime the handle names | NVIDIA / Metal / Vulkan / WebGPU |
...
`Cuda { ordinal, batch_budget }`, `Metal { .. }` and
`Cubecl { device, ordinal, batch_budget }`
