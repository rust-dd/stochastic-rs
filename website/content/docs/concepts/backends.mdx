...
`Metal` and the two CubeCL handles for `f32`, the CPU devices for both. `Fgn<f64>` or
...
| `CubeclCuda`  | `cubecl-cuda`                    | `cubecl` portable kernels on CUDA      | NVIDIA |
| `CubeclWgpu`  | `cubecl-wgpu`                    | `cubecl` portable kernels through wgpu | Metal / Vulkan / WebGPU |
...
let on_second_gpu = gbm.on_device(Cuda::new(1).with_batch_budget(256 << 20));
```

`Cuda { ordinal, batch_budget }`, `Metal { .. }`, `CubeclCuda { .. }` and
`CubeclWgpu { .. }`
...
Apple GPUs have no `f64`, so `Metal` is **`f32` only**, and so is the
CubeCL pipeline: both implement the capabilities for `f32` alone, and an `f64`
process on them is a compile error. `Cuda`, `Accelerate` and `Cpu`
