...
description: Compile-time device selection via `.on::<B>()` and `with_backend(handle)` — the `Backend` marker, the `HostBackend` / `FgnBackend` / `SheetBackend` / `EulerBackend` capabilities, one fallback rule, no run-time cost.
category: concept
since: 2.4.0
status: stable
---
...
Every process selects **where** it samples — CPU, CUDA, Metal, a CubeCL
runtime or Apple Accelerate — at **compile time**. A process is parameterised
