...
Re-type the process by handing `.on` a backend handle, then sample as usual.
...
`B`'s default handle; `.on::<B>()` takes an explicit one:

```rust
use stochastic_rs::stochastic::device::Cuda;

let on_second_gpu = gbm.on(Cuda::new(1).with_batch_budget(256 << 20));
```
