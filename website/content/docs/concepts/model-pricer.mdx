...
- **Multi-asset payoffs** (`MargrabePricer`, `KirkSpreadPricer`,
  ...
  and maturity per call, and every Monte Carlo member returns an
  `McEstimate` — the mean plus its standard error and `ci_95()` — rather
  than a bare number. `pricer_registry.rs` records the state of each.
