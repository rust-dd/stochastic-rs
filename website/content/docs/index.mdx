...
| | | |
|:--|:--|:--|
| **131** processes | **107 G** steps a second | **3 054** tests |
| one trait, one device story | folded in the kernel, on an M4 Max | laws, devices, reproducibility |
...
horizon, seed — and every one gives you `sample`, `sample_par` and the mapped
and folded forms.
...
**Move it to a GPU, and fold it there.** `.on::<Metal>()` — or `device="cuda"`
from Python — is the only change. `sample_reduce` returns one value a path
instead of the whole grid, so the kernel does the fold and the bus carries a
hundred thousand numbers rather than a hundred million.
...
let terminal: Vec<f32> = gbm.on::<Metal>().sample_reduce(100_000, Reduce::Terminal);
```
...
```python
# `device=` is the same choice; the kernel-side fold is Rust-only for now,
# so Python takes the batch back and folds it with numpy.
gbm = srs.PyGbm(0.05, 0.2, 1024, x0=100.0, t=1.0, seed=7, device="cuda", dtype="f32")
terminal = gbm.sample_par(100_000)[:, -1]
```
...
the grid; ask for a borrowed view of it and nothing is copied; ask for a fold
and the kernel never writes it. That last shape is worth 43× the CPU on an
M4 Max and 18–102× the unfolded call on a Tesla T4. The numbers, the method,
and what *did not* work are in [Benchmarks](/docs/benchmarks).
