...
candle runs on the CPU by default. The `ai-metal` (macOS) and `ai-cuda`
features of the umbrella crate compile the matching candle back-end, and
`ai::device::best_available()` returns the fastest device the build can
reach — pass it to any surrogate constructor instead of `Device::Cpu`.
