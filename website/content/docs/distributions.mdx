---
title: Distributions
description: 23 of 33 SIMD distribution structs with full Python + closed-form parity. Bulk samplers; ziggurat, rejection, inversion.
...
The `stochastic-rs-distributions` crate ships **33** `Simd*` distribution
structs (`grep -rc "^pub struct Simd" stochastic-rs-distributions/src --include='*.rs'`),
plus `ComplexDistribution` (34 total). This page's table covers the
**23** that carry the full package — every one of them has:
...
the full package (23 + 10 = 33), plus `ComplexDistribution` outside that
count entirely:
...
That accounts for all 33. `ComplexDistribution` is a 34th, separate type
