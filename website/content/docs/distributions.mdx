---
title: Distributions
description: 18 of 29 SIMD distribution structs with full Python + closed-form parity. Bulk samplers; ziggurat, rejection, inversion.
...
The `stochastic-rs-distributions` crate ships **29** `Simd*` distribution
structs (`grep -rc "^pub struct Simd" stochastic-rs-distributions/src --include='*.rs'`),
plus `ComplexDistribution` (30 total). This page's table covers the
**18** that carry the full package — every one of them has:
...
**Not shown here** — 11 more real, public `Simd*` types that don't carry
the full package (18 + 11 = 29), plus `ComplexDistribution` outside that
count entirely:

- `Ged`, `Gev`, `Skellam`, and the four `Truncated{Normal,Exp,Beta,Gamma}`
  wrappers (7) implement `DistributionExt` but have no Python binding.
- `SimdExpZig` is the ziggurat-table `Exp(1)` primitive that
  ...
  type, not a distinct 19th continuous distribution.
...
That accounts for all 29. `ComplexDistribution` is a 30th, separate type
...
| Extreme-value / block maxima | `Gev` — a different job from bulk return modeling; only `pdf`/`cdf` are closed-form here, not the moments |
...
`DistributionExt` coverage is uneven: `Ged`, `Gev`, `Skellam` and the four
