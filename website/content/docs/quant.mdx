...
| **Calibration**    | Heston (Cui analytic Jacobian + NMLE / PMLE / NMLE-CEKF seeds), Heston SLV (Guyon–Henry-Labordère particle leverage on a Dupire or supplied local volatility, at any mixing fraction), SABR per-expiry caplet smile, Lévy (CGMY, VG, NIG, Merton-jump, Kou, bilateral gamma), SVJ, rough Bergomi, double-Heston, BSM (multi-maturity), HKDE, Hull-White (Jamshidian), G2++ and Black-Karasinski (trinomial-tree repricing) swaption grids via Basin Nelder-Mead. Optional Tikhonov regularisation toward a parameter anchor on the Heston, SABR and swaption calibrators. |
...
| A vanilla surface plus exotics that need the right mix of local and stochastic volatility | `HestonSlvCalibrator` (Heston fit → Dupire local vol → Guyon–Henry-Labordère particle leverage) | reprices the surface at any mixing fraction $\eta$ |
...
mixing fraction $\eta$ is calibrated by the Guyon–Henry-Labordère particle
method — a Gaussian kernel regression of $\mathbb{E}[V_t \mid S_t]$ over an
interacting particle cloud. The result reprices the input grid from its own
cloud and hands a rate-anchored `HestonSlvPricer` back through `ToModel`;
the same surface drives the `HestonSlv` process for exotics.
