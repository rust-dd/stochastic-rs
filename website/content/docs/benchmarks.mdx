...
The CubeCL columns in the tables below are historical: the CubeCL backend
was removed in 3.0.0-rc.2, having duplicated the native CUDA and Metal kernels
at a lower speed, and the benches that produced those columns (`fgn_cubecl`,
`fgn_cuda_compare`) went with it. The other columns stand.

**Apple M4 Max** (10 P + 4 E cores, 36 GB unified; measured 2026-09-03 on the
rc.1 code, after the per-size device caches and the client reuse landed):

| n      | CPU       | Accelerate | Metal      | CubeCL-wgpu |
|-------:|----------:|-----------:|-----------:|------------:|
|  1,024 | **12 µs** |    12 µs   |     126 µs |      281 µs |
|  4,096 |    53 µs  |  **48 µs** |     136 µs |      336 µs |
| 16,384 |   219 µs  |    180 µs  | **168 µs** |      443 µs |
| 65,536 |   901 µs  |    750 µs  | **323 µs** |      772 µs |

**Intel i9-285K + RTX 4070 SUPER** (`cudarc` + cuFFT, hand-written cubecl):

| n      | CPU (i9-285K) | cuFFT (`cuda`) | cubecl (`gpu_cuda`) |
|-------:|--------------:|----------------------:|--------------------:|
...
`n = 65 k`. The CubeCL FFT is the slowest single-path option on both machines,
but no longer by an order of magnitude: reusing one compute client instead of
re-creating it per size cut its `n = 65 k` path from 1.66 ms to 0.77 ms.

### Batch throughput (`sample_par`, `f32`, H = 0.7) — Melem/s, higher is better

**Apple M4 Max** (2026-09-03, rc.1 code) — the four back-ends are within a
factor of two of each other everywhere; Metal leads the small and mid batches,
CubeCL-wgpu the large ones, and the CPU / Accelerate pair holds 530–630 Melem/s
throughout:

| n × m           | CPU | Accelerate | Metal   | CubeCL-wgpu |
|-----------------|----:|-----------:|--------:|------------:|
| 4,096 × 32      | 414 |    **446** |     437 |         214 |
| 4,096 × 128     | 585 |        628 | **700** |         440 |
| 4,096 × 512     | 597 |        597 |     696 |     **714** |
| 16,384 × 128    | 596 |        577 | **690** |         632 |
| 16,384 × 512    | 602 |        535 |     489 |     **800** |
| 1,024 × 1,024   | 532 |        572 | **693** |         580 |
| 4,096 × 16,384  | 591 |        585 |     474 |     **706** |
| 16,384 × 16,384 | 568 |        526 |     421 |     **594** |
| 65,536 × 16,384 | **530** |    464 |     288 |           — |

The CubeCL-wgpu column moved from 58–76 Melem/s to 214–800 between rc.0 and
rc.1: the sampler used to open a new compute client whenever the batch shape
changed, and now keeps one per device. The Metal FFT plans and buffers are
cached for the last four shapes as well. The CPU code path is unchanged; its
numbers sit 10–20 % under the earlier table, which is the run-to-run and
thermal spread of a laptop, not a regression (the CPU fGN output is
bit-identical to rc.0 by test). The largest CubeCL-wgpu case exceeds wgpu's
buffer limits and is skipped.
...
(Mac: 706 and 594 Melem/s on CubeCL-wgpu; i9 + 4070: 581 and 183 on cuFFT), so
...
the GPU back-ends now win batches by 15–35 % rather than losing them. A
