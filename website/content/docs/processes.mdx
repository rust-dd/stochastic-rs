...
paths of a scalar diffusion on backend `B`: `Cpu` always (the crate's SIMD
normal generator, one rayon task per path block), `CubeCl` when the crate is
built with `gpu-cuda` or `gpu-wgpu` (one device thread per path, the whole
recursion in the kernel, counter-hashed Box–Muller normals). A process joins
the engine by describing its coefficients as an `EulerSpec` through
`EulerCoefficients` — `Gbm`, `Ou` and `Cir` do. The stepper is the same
formula on every device; the random streams differ, so CPU and GPU paths agree
in distribution rather than bit for bit. The device works in `f32`, the
portable GPU float, and widens on the way back.
...
# srs.euler_paths(..., device="gpu") needs a build with gpu-cuda / gpu-wgpu
```
</Tab>
</Tabs>
