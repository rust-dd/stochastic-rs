...
`euler::sample_paths::<T, B, _>(&process, m, seed)` runs `m` Euler–Maruyama
paths of a scalar diffusion on backend `B`. `Cpu` (and `Accelerate`) always:
the crate's SIMD normal generator, one rayon task per path. The GPU back-ends
run one device thread per path with the whole recursion in the kernel and
counter-hashed Box–Muller normals:
...
through `EulerCoefficients` — `Gbm`, `Ou` and `Cir` do. The stepper is the
same formula everywhere and the device kernels share one integer hash for
their uniforms, so the device back-ends agree with each other seed for seed
up to libm rounding; the CPU draws its own stream, so CPU and device paths
agree in distribution rather than bit for bit.
