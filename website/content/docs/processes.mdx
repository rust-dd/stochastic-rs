...
Backends: `Cpu` (default), `Cuda` (`cuda`), `Metal` (`metal`), `Cubecl`
(`cubecl-cuda` / `cubecl-wgpu`, the runtime named on the handle),
...
|---|---|---|---|
| `Cubecl` | `cubecl-cuda` / `cubecl-wgpu` | CUDA, or Metal / Vulkan / WebGPU, through CubeCL | `f32` |
