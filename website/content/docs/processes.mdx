...
description: 132 stochastic processes — diffusion, jump, volatility, interest-rate, fractional / rough, and noise. Catalog organised by mathematical family.
category: process
since: 2.0.0
status: stable
---
...
The Heston model under a leverage function $L(t, S)$ and a mixing fraction
$\eta$: $dS_t = \mu S_t\,dt + L(t, S_t)\sqrt{V_t}\,S_t\,dW^1_t$,
$dV_t = \kappa(\theta - V_t)\,dt + \eta\sigma\sqrt{V_t}\,dW^2_t$. The leverage
...
converts into — keeps the process on the host. With $\eta = 1$ and
$L \equiv 1$ the path is `HestonLog`'s, bit for bit. The
[quant page](/docs/quant#heston-slv-leverage-calibration-by-the-particle-method)
calibrates $L$ to a vanilla surface.

<Tabs items={['Rust', 'Python']}>
<Tab value="Rust">
<RustExample path="tests/doctest_processes_heston_slv.rs" />
</Tab>
<Tab value="Python">
```python
import stochastic_rs as srs
...
Backends: `Cpu` (default), `Cuda` (`cuda`), `Metal` (`metal`),
