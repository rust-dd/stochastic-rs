...
The sampling backend is a **type parameter of the process**, exactly as for
the fGN-driven types: `Gbm<T, S, B = Cpu>`, `Ou` and `Cir` are re-typed with
`.on::<B2>()` and then sampled through `ProcessExt` as usual —
`gbm.on::<MetalNative>().sample_par(m)`. On `Cpu` (and `Accelerate`) that is
the process's own sampler, so nothing is re-implemented on the host: GBM keeps
its exact log-normal scheme, OU and CIR their SIMD steppers. The GPU back-ends
...
The device seed is drawn from the process's own seed source (`Deterministic`
reproduces its device paths call after call, `Unseeded` draws fresh entropy).
...
distribution rather than bit for bit. A process joins the engine by describing
its coefficients as an `EulerSpec` through `EulerCoefficients` — `Gbm`, `Ou`
and `Cir` do.
