...
`CudaNative` only exists when the `cuda` feature is compiled, which
...
Backends: `Cpu` (default), `CudaNative` (`cuda`), `CubeCl`
...
| `CudaNative` | `cuda` | NVIDIA, cudarc + NVRTC | `f32` or `f64`, after `T` |
...
# device="gpu" picks the first compiled device back-end; "cuda", "metal"
