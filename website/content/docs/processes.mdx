...
description: 131 stochastic processes — diffusion, jump, volatility, interest-rate, fractional / rough, and noise. Catalog organised by mathematical family.
category: process
since: 2.0.0
status: stable
---
...
The `stochastic-rs-stochastic` crate ships **131 processes** organised
...
counting those inflates 131 to 134, which is how earlier audits of this
...
What the engine runs on today, per backend and precision, is tabulated on
[GPU support](/docs/concepts/gpu-support).

The sampling backend is a **type parameter of every process**
(`Name<T, S, B = Cpu>`, switched with `.on::<B2>()`); the Euler engine is the
capability behind the device markers for `Gbm`, `Ou` and `Cir`:
`gbm.on::<MetalNative>().sample_par(m)` samples through `ProcessExt` as usual.
