...
Backends: `Cpu` (default), `Cuda` (`cuda`), `CubeclCuda` (`cubecl-cuda`),
`CubeclWgpu` (`cubecl-wgpu`), `Metal` (`metal`), `Accelerate`
(`accelerate`) — each handle exists only when its feature is compiled, so an
...
|---|---|---|---|
| `CubeclCuda` | `cubecl-cuda` | CUDA through CubeCL | `f32` |
| `CubeclWgpu` | `cubecl-wgpu` | Metal, Vulkan, WebGPU through CubeCL | `f32` |
...
# device= names one backend: "cuda", "metal", "cubecl-cuda", "cubecl-wgpu"
# or "accelerate", each needing the matching cargo feature of the build;
