...
```toml
[dependencies]
stochastic-rs-distributions = "3.0.0-beta.2"   # SIMD distribution sampling
stochastic-rs-stochastic = "3.0.0-beta.2"   # 127 process types
stochastic-rs-copulas = "3.0.0-beta.2"   # bivariate / multivariate copulas
stochastic-rs-stats = "3.0.0-beta.2"   # estimators
stochastic-rs-quant = "3.0.0-beta.2"   # pricing / calibration / vol surface
stochastic-rs-ai = "3.0.0-beta.2"   # neural surrogates (candle)
```
...
let path  = Fgn::<f32, _>::new(0.7, 65_536, None, Unseeded)
    .on::<CudaNative>().sample();          // single path on GPU
let batch = Fgn::<f32, _>::new(0.7, 65_536, None, Unseeded)
    .on::<CudaNative>().sample_par(1024);  // 1024 paths in one launch
```
...
// routes through whichever cubecl backend is active
let path = Fgn::<f32, _>::new(0.7, 65_536, None, Unseeded)
    .on::<CubeCl>().sample();
```
...
binary (the published Windows wheel **omits** the 13 BLAS-backed
