...
```toml
[dependencies]
stochastic-rs-distributions = "2.6"   # SIMD distribution sampling
stochastic-rs-stochastic = "2.6"   # 127 process types
stochastic-rs-copulas = "2.6"   # bivariate / multivariate copulas
stochastic-rs-stats = "2.6"   # estimators
stochastic-rs-quant = "2.6"   # pricing / calibration / vol surface
stochastic-rs-ai = "2.6"   # neural surrogates (candle)
stochastic-rs-viz = "2.6"   # plotly grid plotter
```
...
| _(none — always on)_ | umbrella                | `stochastic-rs-viz`, `plotly`     | `GridPlotter` is an unconditional umbrella dependency, not feature-gated — see [Visualization](/docs/viz) |
...
let path = Fgn::<f32, _>::new(0.7, 65_536, None, Unseeded)
    .on::<CudaNative>()      // CudaNative | CubeCl | MetalNative | Accelerate | Cpu
    .sample();
```
...
[Backends](/docs/concepts/backends) concept page. The per-backend `sample_*`
methods below are the lower-level calls `.on::<B>()` dispatches to.
...
let fgn = Fgn::<f32, _>::new(/* hurst */ 0.7, 65_536, None, Unseeded);
