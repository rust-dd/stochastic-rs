...
//! exact bit patterns must survive the `ProcessExt` → `PathSampler`
//! migration, proving the derived RNG streams are preserved.
//!
...
fn sampler_first_path_matches_sample() {
  ...
  // both derive the Gaussian source identically from the same seed.
...
fn sample_par_returns_distinct_paths() {
  // `sample_par` (the keep-every-path special case of `sample_map`) yields the
  // requested count, each an independent realisation.
  let gbm = Gbm::<f64, _>::new(0.05, 0.2, 64, Some(1.0), Some(1.0), Unseeded);
