...
mod tests {
  ...
  fn gbm_process_ext_with_mle() {
    let gbm = Gbm::new(0.05, 0.2, 2501, Some(100.0), Some(10.0), Deterministic::new(42));
    let path = gbm.sample();
  ...
  fn ou_sample_then_mle_roundtrip() {
    let ou = Ou::new(3.0, 2.0, 0.5, 10001, Some(2.0), Some(10.0), Deterministic::new(77));
    let path: Array1<f64> = ProcessExt::<f64>::sample(&ou);

    let dt = 10.0 / 10000.0;
    let mut ou_fit = Ou::new(1.0, 1.0, 1.0, 100, Some(1.0), Some(1.0), Deterministic::new(0));
    let result = fit_mle(&mut ou_fit, path.view(), dt, DensityApprox::Exact, None);
  ...
  fn cev_process_ext_with_mle() {
    let cev = Cev::new(0.05, 0.3, 0.7, 2501, Some(100.0), Some(10.0), Deterministic::new(42));
    let path = cev.sample();
    assert_eq!(path.len(), 2501);

    let dt = 10.0 / 2500.0;
    let mut cev_fit = Cev::new(0.0, 0.5, 0.5, 100, Some(100.0), Some(1.0), Deterministic::new(0));
