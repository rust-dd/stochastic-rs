...
- `colab_cpu_vs_cuda_paths.ipynb` — the path gallery: eighteen processes
  across eight families sampled twice, once on the host and once on CUDA, with
  the two sets of paths side by side, their terminal laws overlaid, a table of
  how far apart the laws sit in standard errors, and wall time per batch. Open
  it in
