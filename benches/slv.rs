...
use ndarray::Array1;
use stochastic_rs::quant::pricing::slv::{
  HestonSlvParams, HestonSlvPricer, calibrate_leverage,
};
...
fn bench_calibrate_leverage(c: &mut Criterion) {
  ...
  c.bench_function("slv_calibrate_1k_particles", |b| {
    b.iter(|| {
      calibrate_leverage(&params, 100.0, 0.05, 0.0, &spots, &times, &lv, &spots, &times, 1000, 42)
    })
  });
}
...
fn bench_slv_price_call(c: &mut Criterion) {
  ...
  let leverage =
    calibrate_leverage(&params, 100.0, 0.05, 0.0, &spots, &times, &lv, &spots, &times, 2000, 42);
