...
└── stochastic-rs-py/                 — pyo3 cdylib (285 entries: 261 PyO3 classes + 24 pyfunctions, plus 3 classes + 1 pyfunction behind the py `ai` feature (source builds only), across distributions/stochastic/quant/copulas/stats; AI bindings behind the py `ai` feature). Built via `maturin` (see pyproject.toml `[tool.maturin] manifest-path`)
```
...
- `ProcessExt<T>` — stochastic process simulation (`sample`/`sample_par`/`sample_map`, plus the fallible `try_sample`/`try_sample_par` that report a `DeviceError` instead of panicking); lives in `stochastic-rs-stochastic::traits`. **131** concrete implementors (`grep -rn "ProcessExt<T> for\|ProcessExt<T," stochastic-rs-stochastic/src --include='*.rs' | grep -v /traits/ | wc -l`); the exhaustive per-directory breakdown and the guard that keeps it honest live in `stochastic-rs-stochastic/tests/reproducibility_all_processes.rs`
