...
impl PricerExt for GbmMalliavinPricer {
  ...
  fn calculate_call_put(&self) -> (f64, f64) {
    ...
    let t = self.t_eval;
    assert!(t > 0.0 && t < T, "t_eval must be in (0, T)");

    // Malliavin-based conditional call prices C^M(t, S_t^{(i)}) and corresponding S_t^{(i)}
    ...
    let disc_0t = (-self.r * t).exp();
    let avg_c_t = c_t.mean().unwrap();
    let call_0 = disc_0t * avg_c_t;
    ...
    let df_qT = (-q * T).exp();
    let put_0 = call_0 + self.k * df_rT - self.s * df_qT;
