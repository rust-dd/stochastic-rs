...
1. `CLAUDE.md` — the `131 processes` in the layout tree and the `ProcessExt`
   ...
   reproducibility guard — today 140 minus `AdgRow`, `BgmRow`, `CfouParts`,
   `LmmLaunch`, `McgnsLaunch`, `MultiGbmLaunch`, `MultifactorHestonLaunch`,
   `WishartLaunch`, `WuZhangPair` = 131, matching `grep -rhoE "guard!\(" stochastic-rs-stochastic/tests/reproducibility_all_processes/*.rs | wc -l`.
